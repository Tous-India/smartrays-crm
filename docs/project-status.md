...
Leads, Customers, Attendance, Leave, and Location frontend modules are all done, including
the heartbeat/location-ping submission loop (no longer an open gap); next step is filling in
the rest of the frontend module-by-module (Payroll/Transport are reasonable
next candidates), the PWA service worker + Dashboard polish (Phase 9's
