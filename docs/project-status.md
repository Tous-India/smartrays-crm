...
by an automated test suite (510 backend tests — 509 passing, 1 pre-existing unrelated
...
endpoints (§7.4), and, most recently, Leave's half-day/balance/decline/notification additions
(§7.5)). **This closes out every backend phase in the roadmap.** The User Management frontend
...
- `backend/` — package.json, env config, error handling, response envelope, permission
  ...
  fully working `project` module (Project/Task, team assignment, one-`in_progress`-task-per-
  employee constraint, see §7.3; `Invoice` remains a minimal placeholder model — Phase 7 adds
  partial reconciliation onto it, not full invoicing), a **fully built `attendance` module** (check-in/check-out with photo capture via
...
- **2026-07-13** — Two-part task. **Part 1:** confirmed `getUserById`'s self-fetch bypass was
  ...
  reimplementing it. One open check-in per employee at a time is enforced server-side, the same
  "reject the second one" pattern as the one-`in_progress`-task-per-employee rule (§6.4). No
